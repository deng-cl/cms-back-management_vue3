...
export function handleMenuDataList(menuData: any[]) { // 🔺该函数用于处理 Menu 菜单列表的数据 --> 将服务器响应的菜单数据转换成方便客户端在 table 中展示的数据 <Menu 组件等页使用>
    const menuList: any[] = []

    menuData.forEach(info => { // 一级
        const menu: any = {
        ...
        info?.children.forEach((child: any) => { // 二级
            const permission: any[] = []


            if (child?.permission) {
                Object.keys(child.permission).forEach(key => { // 三级
                    permission.push({
...
export function getButtonPermissionAllByMenu(formatedMenuData: any) { // 根据 Menu 列表获取，遍历出所有按钮权限 <该用户所有按钮权限> --> formatedMenuData 格式化后的 Menu 列表 <即 ↑ handleMenuDataList 后的 Menu 列表>
...
export function isExitsPermission(permission: string) { // 根据权限字段 permission 判断用户是否存在对应的该 permission 权限 <根据 buttonPermission 进行判断>
