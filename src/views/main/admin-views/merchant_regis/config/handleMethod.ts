import { queryPost } from '@/service/modules/main'
import showMessage from '@/utils/showMessage'
import useTableConfig from './useTableConfig'
import type { SelectEnable } from '@/base-ui/h-table/types'

export const selectEnable: SelectEnable = (row, rowIndex) => {
    // 用于决定左侧选框是否为可选状态
    if (row.state === 'true') {
        // 当该行 row 数据中的 state 为 true 时，即已经注册完成，左侧选框禁用
        return false
    }
...
export const registerMerchaantUserList = (selectedList: any[]) => {
    // 注册用户: selectedList 为所选中的用户列表 <单个/多个>
    ...
    payload_list.forEach(async (info) => {
        // 批量注册
...
export const registerMerchaantUser = async (userInfo: any) => {
    // <单个>
