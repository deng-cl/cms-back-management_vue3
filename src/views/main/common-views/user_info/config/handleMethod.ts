...
export async function editorPopupComfirm() {
    ...
    try {
        ...
        Object.keys(userInfo).forEach((key) => {
            if (!userInfo[key] && key !== 'shop_name' && key !== 'id_card') {
                // 店铺名称 + 身份证 --> 可选
        ...
        // 校验表单数据合法性
        if (
            !RULE_TEST_FUN(rules.username, userInfo.username) ||
            !RULE_TEST_FUN(rules.name, userInfo.name) ||
            !RULE_TEST_FUN(rules.phone, userInfo.phone) ||
            !RULE_TEST_FUN(rules.nickname, userInfo.nickname) ||
            !RULE_TEST_FUN(rules.age, userInfo.age)
        )
