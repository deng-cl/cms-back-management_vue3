...
import { queryDelete, queryPost, queryPut } from "@/service/main/main"
...
export async function updateShopState(selectedList: any[], state: 'true' | 'false') { // 🔺修改商品状态 <上架/下架>
    ...
    selectedList.forEach(async info => { // 批量下架/下架
    ...
    payload_list.forEach(shopID => { // 批量下架/下架
        fetchPromiseArray.push(queryPut("/product/update/state", {
            id: shopID,
            state
        }))
    })

    Promise.all(fetchPromiseArray).then(res => {
...
export async function createShop() { // 🔺 创建商品
    console.log("hong")
    const createShopInfo = store.popupFormData
    console.log(createShopInfo)
    try {
        Object.keys(createShopInfo).forEach(key => {
            if (!createShopInfo[key] || createShopInfo[key] === '' && /* 排除 pics 字段 */ key !== 'pics') { // 参数信息不完整
                throw new Error()
            }
        })
        ...
        if (!RULE_TEST_FUN(rules.product_name, createShopInfo.product_name) || !RULE_TEST_FUN(rules.product_desc, createShopInfo.product_desc) || !RULE_TEST_FUN(rules.product_discount, createShopInfo.product_discount)) return // 当表单数据验证不通过时 --> 直接返回，执行新下面请求逻辑
        ...
        if (confirm("请再次确认创建商品信息!")) {
            ...
            await queryPost("/product/create/picture/" + product_id, _formData, { // 将 FormData 数据上传至服务器
...
export async function editorShopInfo() { // 修改商品信息
    ...
    if (confirm("请再次确认修改商品信息")) {
        try {
            ...
            // 2. 修改详情图信息
            store.popupFormData.detail_pics.forEach((item: any) => { // 取到每一个详情图的 filename
            ...
            await queryPut("/product/update/picture/" + updateShopInfo.id, _formData, { // 将 FormData 数据上传至服务器
                "Content-Type": "multipart/form-data"
            })
...
export async function deleteShopInfo(rowData: any) { // 删除商品
...
export function showProductDetail(rowData: any) { // 查询某一商品详情
