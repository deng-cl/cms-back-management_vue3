...
import { queryDelete } from '@/service/main/main'
...
const useTableConfig: UseTableConfig = () => {
    ...
    const handleConfig: DefaultHeaderHandleConfig = {
        ...
        click() {
            ...
            store.popupFormData = { // 当点击新建按钮时，用于创建标签，重置 popup 中的 formData <避免 formData 遗留前面编辑的哪一行数据...>
                tag: undefined,
                msg: undefined
            }
    ...
    const handleColumnEvnet: HandleColumnEventType = {
        editorClickEvent(rowData: any) { // 编辑/修改商品标签
            store.showPopupType = 'editor' // 将 popup 配置类型改为 editor 编辑类型
            store.popupFormData = { // 当点击编辑时，将 store 中的 popupFormData 状态修改为当前行的数据 --> 用于方便弹出层表单展示所要编辑的数据 <修改 popup 中的 formData>
        ...
        async deleteClickEvent(rowData: any) { // 删除商品标签
            const { id, tag_name } = rowData // 1. 获取所触发 delete 事件的改行标签数据

            if (confirm(`请再次确认删除 "${tag_name}" 标签`)) {
                await queryDelete(`/product/delete/tag/${id}`, {}) // 2. 发送请求 --> 删除对应标签
