...
import { queryGet, queryPost } from "@/service/main/main"
import { ref, computed, type Ref } from "vue"

export interface StoreReturnType { // 定义 store 返回的状态属性类型
    userInfo: Ref<any>
...
export const useCommonStoreConfig: CommonStoreConfigType = (url: string, formData: any, isGet: boolean = false) => {
    return () => {
        ...
        const activeUserInfo = computed(() => { // 当前所在页面展示的用户列表信息 --> 当前展示 userInfo 列表 <计算属性: 当前 currentPage 当前页标识发生改变时，自动计算当前页所要展示的数据>
        ...
        const sizeChange = (sizeCount: any) => { // 监听更改页面展示的数据量的修改事件 --> 用于需改单页数据展示量...
            size.value = sizeCount
            reStateByQueryList() // 状态重置
            queryList(formData) // 当 size 数据展示量发生改变时，重新发起数据的请求
        }

        const currentChange = (pageCount: any) => { // 监听当前页的标识事件 --> 即当前页发生改变时 --> 如 1 - 2 ... --> 用于修改当前页时，重新请求更新数据或切换数据...

            currentPage.value = pageCount // 更新当前页码

            if (!userInfo.value[pageCount] /* 不存在数据 */) { // 判断 userInfo 中是否已经存在该 pageCount 页的数据，不存在则请求，反之使用缓存中的数据
                queryList(formData) // formData 模糊查询的参数<在 ./config 中进行导入> --> 重新请求数据
            }
        }

        const queryList = async (payload: any) => { // 数据请求方法
            let result = null
            if (isGet) {
        ...
        const reStateByQueryList = () => { // store 状态重置: 点击搜索按钮... --> 重置 pages、userInfo、currentPage
            userInfo.value = [] // 重置 userInfo 数据信息
            currentPage.value = 1 // 重置 currentPage 当前页码
        }

        const paginationConfig: Ref<PaginationConfig> = ref({ // 分页器配置项
            prop: {
                total: total,
                size: size.value,
