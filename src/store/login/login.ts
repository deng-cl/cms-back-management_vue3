...
import { getMenuList, submitLogin } from "@/service/login/login"; // 发送登录请求
...
const useLoginStore = defineStore("login", (): LoginStoreType => {
    ...
    watch(isSavePass, (n) => { localCache.setCache("isSavePass", n) /* 是否记住密码选项持久化处理 */ })
    ...
    const submitAccountLogin = async (payload: AccountRuleForm) => { /* Account 登录请求等逻辑 */
        const result = await submitLogin(payload) // 服务器返回的数据
        /* 保存 token 信息 */
        if (result) { // 登录请求成功 -- 有值的情况下
    ...
    const submitPhoneLogin = (payload: any) => { /* Phone 登录请求等逻辑 */
        console.log("执行 submitPhoneLogin --> ", payload);
    }

    const upBreadcrumb = (path: string) => [ /* 更新 breadcrumb 面包屑数据 */
        breadcrumb.value = getBreadcrumbDate(path)
    ]
