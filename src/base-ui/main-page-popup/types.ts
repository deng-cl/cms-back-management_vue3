...
type PopupFormType = 'input' | 'password' | 'checkbox' | 'radio' | 'number' | 'input-number' | 'select' | 'switch' | 'upload'  // 所封装的表单列表


export interface SelectOptionType { // 针对 select 的主要依赖配置项类型
    label: string
    value: string | number | undefined
}

export interface RadioOptionType { // 针对 radio 的主要依赖配置项类型
    label: string
    value: string | undefined
}


export interface PopupFormItem { // 🔺单个表单项的配置
...
export interface PopupForm { /* 🔺整个 hForm 自定义表单的配置项整合 */
